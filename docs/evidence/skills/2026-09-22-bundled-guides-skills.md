...
The feature branch built on source `dc754dfe42142e6e895e9f1f151ea5fc65b3e468` (merged PR #28 head). The verification harness built and installed the wheel in the isolated run `skills-disc-20260922` under `/tmp/html-publish-verify/`. The doctor check passed before driving: owner and installed executable matched, and the health endpoint answered

| Identity | Value |
| --- | --- |
| Installed wheel release | `sha256-92ef377172c920549bfaed9dace854d4620748b787e4723cf1ca340ccb79a26a` |
| Bundled core guide | `sha256-a72f7e1f9e0cc540dd9137df4faabf781067800387322ec3bbab306672458c8f`, 8,823 bytes |
| Bundled recovery guide | `sha256-57a3b2b803e2d4663e41084e45e04dd90b41a06fc53d7f7ea8934bb89d7a2fc8`, 5,819 bytes |
| Executable version | `0.1.0` |
| Verification feature | [skills discovery](../../../.agents/skills/verify-html-publish/features/skills-discovery.md), run `skills-disc-20260922` |
...
The Linux proof drove the installed executable from the run's virtual environment, outside the source checkout. The captured transcript is retained as `skills-discovery-skills-disc-20260922.txt` in the run's artifacts directory at `/tmp/html-publish-verify/skills-disc-20260922/artifacts/`. Observed results:

| Check | Observed result |
| --- | --- |
| `skills list` | Exit 0. One JSON object with `schema_version` `1`, `executable` `html-publish`, `version` `0.1.0`, and guides `core` then `recovery` with byte counts 8,823 and 5,819 |
| `skills get core` | Exit 0. Stdout started with `# Core guide` and carried the `html-publish 0.1.0` version marker |
| Byte parity | Inventory `bytes` equaled the packaged file size and the plain stdout byte count for both guides (8,823 and 5,819) |
| `skills get recovery --json` | Exit 0. One object with `schema_version` `1`, `name` `recovery`, `version` `0.1.0`, and 5,819 content bytes |
...
The same wheel was copied to `m4mini.donkey-arcturus.ts.net` (macOS 26.6.2, arm64) over SSH with strict host-key checking. A disposable `uv` virtual environment with Python 3.12 installed the wheel from `/tmp`, and the installed executables ran the same checks. The captured transcript is retained as `skills-discovery-m4mini-skills-disc-20260922.txt` in the run's artifacts directory

| Check | Observed result |
| --- | --- |
| Install identity | Installed guide hashes matched the reviewed bytes: core `a72f7e1f…`, recovery `57a3b2b8…` |
| `skills list` | Identical JSON to the Linux run, including byte counts 8,823 and 5,819 |
| `skills get core` | Head matched the reviewed guide text with the `html-publish 0.1.0` marker |
| Byte parity | Core stdout 8,823 bytes; recovery stdout 5,819 bytes |
| `skills get recovery --json` | `name` `recovery`, `version` `0.1.0`, `schema_version` `1`, 5,819 content bytes |
