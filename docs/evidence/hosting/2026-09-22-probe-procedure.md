...
SIGTERM and SIGINT request cancellation at a safe ownership boundary. The probe stops bounded command groups and retains failure evidence. Cancellation before server creation clears the startup intent. Cancellation during creation lets the parent record the exact child identity before stopping it. Cancelling a later checkpoint leaves the persistent server for explicit cleanup. Run the cleanup command after cancellation. These guarantees do not cover SIGKILL or power loss
