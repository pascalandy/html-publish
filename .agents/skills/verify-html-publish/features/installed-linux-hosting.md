...
- `host-ownership` rejects equal unowned units and Serve handlers.
...
- Add `--tailscale` only on an already-authenticated disposable node with private HTTPS enabled.
...
- **Private HTTPS.** Require a separate authenticated disposable node and second tailnet client.
  No current automated job proves this path.
...
- A simulated Serve state or generated route text does not prove private HTTPS.
