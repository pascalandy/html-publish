...
- **Prove deletion recovery in the focused fault fixture.** Run `uv run python -m unittest -v tests.test_cli.PublisherCliTest.test_identical_retry_preserves_known_removed_path_verification tests.test_cli.PublisherCliTest.test_file_and_directory_transitions_appear_in_plan_differences`. The original-expectation retry must fail while the removed URL returns 200, with both effects false and the same B commit. After route repair, it must return `unchanged`, passed `removed_paths`, and two page commits. The fixture also covers absent, current, unknown, and other-publication expectations plus the file-to-directory exemption.
...
- The fault fixture drives the source checkout and its own loopback server. The installed-wheel retries above are the live CLI proof.
